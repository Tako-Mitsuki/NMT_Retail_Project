...
# --- 1. Tạo cột 'Segment' nếu chưa có ---
if "Segment" not in rfm.columns:
    # Quy ước: top 1% khách hàng có Monetary cao nhất là VIP
    vip_threshold = rfm["Monetary"].quantile(0.99)
    rfm["Segment"] = np.where(rfm["Monetary"] > vip_threshold, "VIP", "ToCluster")
    print("✅ Cột 'Segment' đã được tạo (top 1% Monetary = VIP).")
else:
    print("⚙️ Cột 'Segment' đã tồn tại, bỏ qua bước tạo mới.")

# --- 2. Lọc khách hàng không phải VIP ---
rfm_cluster = rfm.loc[rfm["Segment"].eq("ToCluster"), ["Recency", "Frequency", "Monetary"]].copy()

# --- 3. Chuẩn hoá dữ liệu ---
scaler = StandardScaler()
rfm_scaled = scaler.fit_transform(rfm_cluster)

# --- 4. Nếu có cột Cluster, tính trung bình theo cụm ---
if "Cluster" in rfm.columns:
    cluster_profile = rfm.groupby("Cluster")[["Recency", "Frequency", "Monetary"]].mean()
    print("📊 Trung bình RFM mỗi cụm:")
    print(cluster_profile)

    # Gắn nhãn cụm sang tên dễ hiểu
    cluster_labels = {
        0: "Khách giá trị cao",
        1: "Khách trung thành",
        2: "Khách mới",
    ...
    print("⚠️ Cột 'Cluster' chưa có — cần chạy bước phân cụm trước.")
